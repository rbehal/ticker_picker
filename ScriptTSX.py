...
def ao5(df):
    ...
    df = pd.DataFrame(np.array([ao5]), columns=df.columns).append(
        df, ignore_index=False)
    df = df.rename(index={0: 'Average of 5'})
...
raw_data = pd.read_csv(
    filepath_data + ticker + ".csv", index_col=0)
predicted_data_raw = stock_predict_HC(raw_data)
predicted_data_preprocessed = pd.DataFrame(
    predicted_data_raw['Overall Rating'])
predicted_data_abs_final = predicted_data_preprocessed.rename(
    columns={'Overall Rating': ticker})

#################################################
# constituents = pd.read_csv(
#     '/Users/rohitgarudadri/Desktop/Rohit/Programming/YHack/ticker_picker/constituents_csv.csv')
# ticker = constituents['Symbol'][0]
# raw_data = pd.read_csv(
#     "/Users/rohitgarudadri/Desktop/Rohit/Programming/YHack/ticker_picker/Stock Data/" + ticker + ".csv", index_col=0)
# predicted_data_raw = stock_predict_HC(raw_data)
# predicted_data_preprocessed = pd.DataFrame(
#     predicted_data_raw['Overall Rating'])
# predicted_data_abs_final = predicted_data_preprocessed.rename(
...
for i in range(1, len(stocks_in_tsx_proper)):
    try:
        ...
        predicted_data_final = predicted_data_preprocessed.rename(
            columns={'Overall Rating': ticker})
        predicted_data_abs_final = predicted_data_abs_final.join(
...
data_averaged = ao5(predicted_data_abs_final.copy())
...
def top20(df, position='buy'):
    if position == 'buy':
        ...
        top20_buy = top20_buy.rename(columns={'index':'Ticker','Average of 5':'Score'})
        top20_buy.index += 1
        return top20_buy
    if position == 'sell':
        top20_sell = pd.DataFrame(df.iloc[0,-20:])
        top20_sell = top20_sell.rename(columns={'index':'Ticker','Average of 5':'Score'})
...
topframe.pack()
label = tkinter.Label(
    topframe, text="Enter a ticker to retrieve that stock's data or it's buy/sell indicator status.")
label.pack()
...
indicatorbutton = tkinter.Button(
    bottomframe, text="Retrieve indicator summary", command=lambda: get_indicators(val.get().upper()))
...
label = tkinter.Label(
    legend, text="Strong Buy (8 to 16)/nBuy (1 to 8)/nNeutral (-1 to 1)/nSell (-8 to -1)/nStrong Sell (-16 to -8)", borderwidth=2, relief="solid")
