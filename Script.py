...
def daily_df(ticker):
    ...
    if response.status_code == 200:
        ...
        daily_dataframe = df_entries2float(daily_dataframe)
...
def ao10(df):
    ...
    df = pd.DataFrame(np.array([ao10]), columns=df.columns).append(
        df, ignore_index=False)
    df = df.rename(index={0: 'Average of 10'})
...
data_averaged = ao10(predicted_data_abs_final.copy())
...
label = tkinter.Label(
    legend, text="Strong Buy (8 to 16)\nBuy (1 to 8)\nNeutral (-1 to 1)\nSell (-8 to -1)\nStrong Sell (-16 to -8)", borderwidth=2, relief="solid")
